...
@php
    ...
    $hasNonFinal =
        $hasVendorStages &&
        $vendorStages->contains(fn($vs) => !in_array($vs->stage?->type, ['deliver', 'cancel', 'refund']));
    $allFinal =
        $hasVendorStages &&
        $vendorStages->every(fn($vs) => in_array($vs->stage?->type, ['deliver', 'cancel', 'refund']));
...
@if ($order)
    ...
    <div class="d-flex gap-2 flex-wrap align-items-center {{ $context === 'show' ? 'justify-content-center' : '' }}">
        ...
        @if ($showViewButton && $context === 'list')
            @can('orders.view')
                <a href="{{ route('admin.orders.show', $order->id) }}" class="btn btn-primary table_action_father"
        ...
        @if (isAdmin())
            ...
            @if ($allNew)
                @can('orders.update')
                    <a href="{{ route('admin.orders.edit', $order->id) }}" class="btn btn-info table_action_father"
            ...
            @if (!$allFinal && count($orderStages) > 0)
                <button type="button" class="btn btn-warning table_action_father btn-admin-change-stage"
                    data-order-id="{{ $order->id }}" data-current-stage-type="{{ $adminCurrentStageType }}"
            ...
            @if ($hasVendorStages)
                ...
                @if ($allNew)
                    <button type="button" class="btn btn-secondary table_action_father btn-admin-quick-stage"
                        data-order-id="{{ $order->id }}" data-target-type="in_progress"
                ...
                @if ($allInProgress)
                    <button type="button" class="btn btn-success table_action_father btn-admin-quick-stage"
                        data-order-id="{{ $order->id }}" data-target-type="deliver"
                ...
                @if ($hasNonFinal)
                    <button type="button" class="btn btn-danger table_action_father btn-admin-quick-stage"
                        data-order-id="{{ $order->id }}" data-target-type="cancel"
            ...
            @if ($order->payment_type === 'online')
                @can('orders.view')
                    <a href="{{ route('admin.orders.payments', $order->id) }}" class="btn btn-success table_action_father"
            ...
            @if ($hasInProgress)
                <a href="{{ route('admin.order-fulfillments.allocate', $order->id) }}"
                    class="btn btn-info table_action_father" title="{{ trans('order::order.allocate') }}">
        ...
        @if ($isVendorUser && !$isVendorFinalStage)
            {{-- In Progress button (if vendor is new) --}}
            @if ($vendorStageType === 'new')
                <button type="button" class="btn btn-secondary table_action_father btn-vendor-quick-stage"
                    data-order-id="{{ $order->id }}" data-target-type="in_progress"
                    title="{{ trans('order::order.mark_in_progress') }}">
                    <i class="uil uil-process table_action_icon"></i>
                </button>
            ...
            @if (count($orderStages) > 0)
                <button type="button" class="btn btn-warning table_action_father btn-change-vendor-stage"
                    data-order-id="{{ $order->id }}" data-stage-id="{{ $vendorStageId }}"
                    data-stage-type="{{ $vendorStageType }}" title="{{ trans('order::order.change_stage') }}">
                    <i class="uil uil-exchange table_action_icon"></i>
                </button>
            @endif
            ...
            @if ($vendorStageType === 'in_progress')
                <button type="button" class="btn btn-success table_action_father btn-vendor-quick-stage"
                    data-order-id="{{ $order->id }}" data-target-type="deliver"
                    title="{{ trans('order::order.mark_delivered') }}">
                    <i class="uil uil-check-circle table_action_icon"></i>
                </button>
            ...
            @if (!in_array($vendorStageType, ['deliver', 'cancel', 'refund']))
                <button type="button" class="btn btn-danger table_action_father btn-vendor-quick-stage"
                    data-order-id="{{ $order->id }}" data-target-type="cancel"
                    title="{{ trans('order::order.cancel_order') }}">
                    <i class="uil uil-times-circle table_action_icon"></i>
                </button>
...
@once
    @push('after-body')
        ...
        @if (isAdmin() && !empty($orderStages))
            ...
            <div class="modal fade" id="adminQuickStageModal" tabindex="-1" aria-labelledby="adminQuickStageModalLabel"
                aria-hidden="true">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-primary"
                                id="confirmAdminQuickStageBtn">{{ __('common.confirm') }}</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal fade" id="adminChangeStageModal" tabindex="-1" aria-labelledby="adminChangeStageModalLabel"
                aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="adminNewStage"
                                    class="form-label fw-bold">{{ trans('order::order.select_new_stage') }}</label>
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary"
                                data-bs-dismiss="modal">{{ __('common.cancel') }}</button>
                            <button type="button" class="btn btn-primary"
                                id="confirmAdminStageBtn">{{ trans('order::order.update_stage') }}</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        @if (!isAdmin() && !empty($orderStages))
            ...
            <div class="modal fade" id="vendorQuickStageModal" tabindex="-1" aria-labelledby="vendorQuickStageModalLabel"
                aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary"
                                data-bs-dismiss="modal">{{ __('common.cancel') }}</button>
                            <button type="button" class="btn btn-primary"
                                id="confirmVendorQuickStageBtn">{{ __('common.confirm') }}</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal fade" id="vendorChangeStageModal" tabindex="-1" aria-labelledby="vendorChangeStageModalLabel"
                aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary"
                                data-bs-dismiss="modal">{{ __('common.cancel') }}</button>
                            <button type="button" class="btn btn-primary"
                                id="confirmVendorStageBtn">{{ trans('order::order.update_stage') }}</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <script>
            ...
            window.OrderActionsHelper = {
                ...
                generate: function(row, options = {}) {
                    ...
                    if (canView) {
                        html +=
                            `<a href="${showUrl}" class="view btn btn-primary table_action_father" title="${this.translations.viewOrder}"><i class="uil uil-eye table_action_icon"></i></a>`;
                    }

                    // For vendors: show stage change button
                    if (isVendorUser && !isVendorFinalStage) {
                        html +=
                            `<button type="button" class="btn btn-warning table_action_father btn-change-vendor-stage" data-order-id="${row.id}" data-stage-id="${vendorStageId || ''}" data-stage-type="${vendorStageType || ''}" title="${this.translations.changeStage}"><i class="uil uil-exchange table_action_icon"></i></button>`;
                    }

                    // For admin: show stage change button for all vendors
                    if (!isVendorUser && !isFinalStage) {
                        html +=
                            `<button type="button" class="btn btn-warning table_action_father btn-admin-change-stage" data-order-id="${row.id}" title="${this.translations.changeStageAllVendors}"><i class="uil uil-exchange table_action_icon"></i></button>`;
                    }
                    ...
                    if (!isVendorUser && row.vendors_with_stages && row.vendors_with_stages.length > 0) {
                        ...
                        if (allNew && canEdit) {
                            html +=
                                `<a href="${editUrl}" class="btn btn-info table_action_father" title="${this.translations.editOrder}"><i class="uil uil-edit table_action_icon"></i></a>`;
                        }

                        // In Progress button
                        if (allNew) {
                            html +=
                                `<button type="button" class="btn btn-secondary table_action_father btn-admin-quick-stage" data-order-id="${row.id}" data-target-type="in_progress" title="${this.translations.markInProgress}"><i class="uil uil-process table_action_icon"></i></button>`;
                        }

                        // Deliver button
                        if (allInProgress) {
                            html +=
                                `<button type="button" class="btn btn-success table_action_father btn-admin-quick-stage" data-order-id="${row.id}" data-target-type="deliver" title="${this.translations.markDelivered}"><i class="uil uil-check-circle table_action_icon"></i></button>`;
                        }

                        // Cancel button
                        if (hasNonFinal) {
                            html +=
                                `<button type="button" class="btn btn-danger table_action_father btn-admin-quick-stage" data-order-id="${row.id}" data-target-type="cancel" title="${this.translations.cancelOrder}"><i class="uil uil-times-circle table_action_icon"></i></button>`;
                        }
                    }
                    ...
                    if (isVendorUser && vendorStage) {
                        if (vendorStageType === 'new') {
                            html +=
                                `<button type="button" class="btn btn-secondary table_action_father btn-vendor-quick-stage" data-order-id="${row.id}" data-target-type="in_progress" title="${this.translations.markInProgress}"><i class="uil uil-process table_action_icon"></i></button>`;
                        }

                        if (vendorStageType === 'in_progress') {
                            html +=
                                `<button type="button" class="btn btn-success table_action_father btn-vendor-quick-stage" data-order-id="${row.id}" data-target-type="deliver" title="${this.translations.markDelivered}"><i class="uil uil-check-circle table_action_icon"></i></button>`;
                        }

                        if (!isVendorFinalStage) {
                            html +=
                                `<button type="button" class="btn btn-danger table_action_father btn-vendor-quick-stage" data-order-id="${row.id}" data-target-type="cancel" title="${this.translations.cancelOrder}"><i class="uil uil-times-circle table_action_icon"></i></button>`;
                        }
                    }
                    ...
                    if (hasOnlinePayment) {
                        html +=
                            `<a href="${paymentsUrl}" class="btn btn-success table_action_father" title="${this.translations.viewOnlinePayment}"><i class="uil uil-credit-card table_action_icon"></i></a>`;
                    }
                    ...
                    if (!isVendorUser && row.vendors_with_stages && row.vendors_with_stages.length > 0) {
                        ...
                        if (hasInProgress) {
                            html +=
                                `<a href="${allocateUrl}" class="btn btn-info table_action_father" title="${this.translations.allocate}"><i class="uil uil-box table_action_icon"></i></a>`;
                        }
                    }
                ...
                @if (isAdmin())
                    ...
                    $('#confirmAdminQuickStageBtn').on('click', function() {
                        ...
                        $.ajax({
                                ...
                                if (targetType === 'in_progress') {
                                    setTimeout(function() {
                                        window.location.href =
                                            '{{ route('admin.order-fulfillments.allocate', ['orderId' => '__ORDER__']) }}'
                                            .replace('__ORDER__', orderId);
                                    }, 500);
                                } else {
                            ...
                            error: function(xhr) {
                                if (typeof LoadingOverlay !== 'undefined') LoadingOverlay.hide();
                                let errorMessage =
                                    '{{ trans('order::order.error_updating_stage') }}';
                    ...
                    $(document).on('click', '.btn-admin-change-stage', function() {
                        ...
                        if (!currentStageType) {
                            ...
                            if (!dataTable && $.fn.DataTable && $.fn.DataTable.isDataTable(
                                '#ordersDataTable')) {
                            ...
                            if (dataTable) {
                                ...
                                if (rowData && rowData.vendors_with_stages && rowData.vendors_with_stages
                                    .length > 0) {
                        ...
                        if (currentStageType && isFinalStage(currentStageType)) {
                            $('#adminStageWarning').text(
                                '{{ trans('order::order.cannot_change_final_stage') }}').show();
                        } else {
                            // Add only valid transition stages
                            let hasOptions = false;
                            orderStages.forEach(stage => {
                                if (canTransitionTo(currentStageType, stage.type, currentStageId, stage
                                        .id)) {
                                    $select.append(
                                        `<option value="${stage.id}" data-type="${stage.type}">${stage.name}</option>`
                                        );
                                    hasOptions = true;
                                }
                            });
                            ...
                            if (!hasOptions) {
                                $('#adminStageWarning').text('{{ trans('order::order.no_available_stages') }}')
                                    .show();
                            }
                        }
                    ...
                    $('#confirmAdminStageBtn').on('click', function() {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (typeof LoadingOverlay !== 'undefined') LoadingOverlay.hide();
                                bootstrap.Modal.getInstance(document.getElementById(
                                    'adminChangeStageModal')).hide();
                                toastr.success(response.message ||
                                    '{{ trans('order::order.stage_updated_successfully') }}');

                                if (selectedStage && (selectedStage.type === 'in_progress' ||
                                        selectedStage.slug === 'in-progress')) {
                                    setTimeout(function() {
                                        window.location.href =
                                            '{{ route('admin.order-fulfillments.allocate', ['orderId' => '__ORDER__']) }}'
                                            .replace('__ORDER__', orderId);
                            ...
                            complete: function() {
                                $btn.prop('disabled', false).html(
                                    '{{ trans('order::order.update_stage') }}');
                            }
                        });
                    });
                @endif
