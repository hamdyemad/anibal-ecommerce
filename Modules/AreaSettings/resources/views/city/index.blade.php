...
@section('title', __('areasettings::city.cities_management'))
...
@section('content')
                        ...
                        <div class="card border-0 shadow-sm">
                            <div class="card-body">
                                <div class="row g-3 align-items-end">
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label for="country_id" class="il-gray fs-14 fw-500 mb-10">{{ __('areasettings::city.country') }}</label>
                                            <select class="form-control ih-medium ip-gray radius-xs b-light px-15" id="country_id">
                                                <option value="">{{ __('areasettings::city.all_countries') }}</option>
                                                @foreach($countries as $country)
                                                    <option value="{{ $country->id }}">{{ $country->getTranslation('name', app()->getLocale()) ?? $country->code }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label for="active" class="il-gray fs-14 fw-500 mb-10">{{ __('areasettings::city.status') }}</label>
                        ...
                        <table id="citiesDataTable" class="table mb-0 table-bordered table-hover" style="width:100%">
                            <thead>
                                <tr class="userDatatable-header">
                                    ...
                                    @endforeach
                                    <th>
                                        <span class="userDatatable-title">{{ __('areasettings::city.country') }}</span>
                                    </th>
                                    <th>
                                        <span class="userDatatable-title">{{ __('areasettings::city.regions') ?? 'Regions' }}</span>
                                    </th>
                                    <th>
                                        <span class="userDatatable-title">{{ __('areasettings::city.status') }}</span>
                                    </th>
                                    <th>
                                        <span class="userDatatable-title">{{ __('areasettings::city.created_at') }}</span>
                                    </th>
                                    <th>
    ...
    <x-delete-modal 
        ...
        :title="__('areasettings::city.confirm_delete')"
        :message="__('areasettings::city.delete_confirmation')"
        itemNameId="delete-city-name"
        confirmBtnId="confirmDeleteCityBtn"
        :deleteRoute="route('admin.area-settings.cities.index')"
        :cancelText="__('areasettings::city.cancel')"
        :deleteText="__('areasettings::city.delete_city')"
    />
@endsection
        ...
        columns: [
            ...
            { 
                ...
                render: function(data, type, row) {
                    ...
                    return '<div class="userDatatable-content"' + rtlAttr + '><strong>' + (nameData.value || '-') + '</strong></div>';
                }
            },
            @endforeach
            { 
                ...
                render: function(data, type, row) {
                    return `
                        <ul class="orderDatatable_actions mb-0 d-flex flex-wrap justify-content-start">
                            @can('area.city.show')
                            <li>
                                <a href="{{ url('admin/area-settings/cities') }}/${row.id}" class="view" title="{{ trans('common.view') }}">
                            ...
                            @can('area.city.edit')
                            <li>
                                <a href="{{ url('admin/area-settings/cities') }}/${row.id}/edit" class="edit" title="{{ trans('common.edit') }}">
